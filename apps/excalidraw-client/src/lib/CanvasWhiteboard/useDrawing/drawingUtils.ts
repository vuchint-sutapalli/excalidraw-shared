 ...
 */
export const drawLabel = (ctx: CanvasRenderingContext2D, element: Element) => {
	if (!element.label) return;

	const center = getElementCenter(element);
	const label = element.label;

	const FONT_SIZE = element.labelFontSize || 16;
	const FONT_FAMILY = element.labelFontFamily || "sans-serif";
	...
	// Draw background
	ctx.fillStyle = element.labelBackgroundColor || "white";
	ctx.fillRect(rectX, rectY, rectWidth, rectHeight);

	// Draw text
	ctx.fillStyle = element.labelColor || "black";
	label.split("\n").forEach((line, index) => {
		const yPos = center.y - totalHeight / 2 + LINE_HEIGHT * (index + 0.5);
		ctx.fillText(line, center.x, yPos);
	});
	ctx.restore();
};

/**
 * Draws an annotation element, resembling a map marker.
 * @param ctx The canvas rendering context.
 * @param element The annotation element to draw.
 */
export const drawAnnotationElement = (
	ctx: CanvasRenderingContext2D,
	element: AnnotationElement
) => {
	console.log("drawing annotation", element);

	const { x, y, width, height } = element;
	const markerWidth = width || 64;
	const markerHeight = height || 64;
	const isActive = element.isActivelySelected;
	...
	// Marker body (teardrop shape)
	ctx.save();
	ctx.translate(x, y);

	ctx.beginPath();
	ctx.moveTo(0, -markerHeight / 2); // Top point
	ctx.bezierCurveTo(
		markerWidth / 2,
		-markerHeight / 2,
		markerWidth / 2,
	...
	// If active, draw a "glow" and halo effect to highlight it.
	if (isActive) {
		ctx.shadowColor = "rgba(92, 124, 250, 0.9)";
		ctx.shadowBlur = 20;
		ctx.shadowOffsetX = 0;
		ctx.shadowOffsetY = 0;
		ctx.strokeStyle = "rgba(92, 124, 250, 0.5)";
		ctx.lineWidth = 4;
		ctx.stroke();

		// Draw a red line extending to the right to connect to the modal
		ctx.beginPath();
		ctx.moveTo(markerWidth / 2, 0);
		ctx.lineTo(markerWidth / 2 + 40, 0);
		ctx.strokeStyle = "#e03131"; // Excalidraw red
		ctx.lineWidth = 2;
		ctx.setLineDash([]); // Ensure the line is solid
		ctx.stroke();
	}

	//fillStyle is red if element.annotationState is open else green
	const isResolved = element.annotationState === "resolved";
	ctx.fillStyle = isResolved ? "#2f9e44" : "#e03131"; // Excalidraw green/red

	// ctx.fillStyle = element.fill || "#FF4136"; // Default to a noticeable red
	ctx.fill();

	// Clear shadow for inner icon
	ctx.shadowColor = "transparent";
	ctx.shadowBlur = 0;
	ctx.shadowOffsetY = 0;

	// Draw the inner icon
	const innerCircleRadius = markerWidth * 0.2;
	const iconCenterY = -markerHeight * 0.05;

	if (isResolved) {
		// Draw white circle background
		ctx.beginPath();
		ctx.arc(0, iconCenterY, innerCircleRadius, 0, 2 * Math.PI);
		ctx.fillStyle = "white";
		ctx.fill();

		// Draw green checkmark
		ctx.beginPath();
		ctx.moveTo(-innerCircleRadius * 0.5, iconCenterY);
		ctx.lineTo(-innerCircleRadius * 0.1, iconCenterY + innerCircleRadius * 0.4);
		ctx.lineTo(innerCircleRadius * 0.6, iconCenterY - innerCircleRadius * 0.3);
		ctx.strokeStyle = "#2f9e44";
		ctx.lineWidth = 2;
		ctx.stroke();
	} else {
		// show the authorName as an initial at the center of tear drop. found inside element.authorName
		if (element.authorName) {
			const initial = element.authorName.charAt(0).toUpperCase();
			ctx.fillStyle = "white";
			// Adjust font size based on marker size for scalability
			ctx.font = `bold ${markerWidth * 0.5}px sans-serif`;
			ctx.textAlign = "center";
			ctx.textBaseline = "middle";
			ctx.fillText(initial, 0, iconCenterY);
		}
	}
...
export const drawElement = (
	...
	// const { activeAnnotationId } = useWhiteboard();
	...
	if (el.type === "rectangle") {
		...
		if (el.points && el.points.length > 1) {
			...
			if (el.cp2x && el.cp2y) {
				// Cubic Bezier curve
				ctx.moveTo(el.x, el.y);
				ctx.bezierCurveTo(el.cp1x, el.cp1y, el.cp2x, el.cp2y, el.x2, el.y2);
				arrowAngle = Math.atan2(el.y2 - el.cp2y, el.x2 - el.cp2x); // Tangent at t=1
			} else {
		...
		drawAnnotationElement(ctx, el as AnnotationElement);
	}
