...
export const getQuadraticCurveBounds = (p0: Point, p1: Point, p2: Point) => {
	...
	const tdx = p0.x - 2 * p1.x + p2.x;
	if (tdx !== 0) {
		const tx = (p0.x - p1.x) / tdx;
		if (tx > 0 && tx < 1) {
			const x = getPointOnQuadraticCurve(tx, p0, p1, p2).x;
			minX = Math.min(minX, x);
			maxX = Math.max(maxX, x);
		}
	}

	const tdy = p0.y - 2 * p1.y + p2.y;
	if (tdy !== 0) {
		const ty = (p0.y - p1.y) / tdy;
		if (ty > 0 && ty < 1) {
			const y = getPointOnQuadraticCurve(ty, p0, p1, p2).y;
...
export const getPointOnQuadraticCurve = (
	t: number,
	p0: Point,
	p1: Point,
	p2: Point
